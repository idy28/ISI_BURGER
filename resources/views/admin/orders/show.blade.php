...
@section('title', 'Commande #' . str_pad($commande->id, 3, '0', STR_PAD_LEFT))
  @section('page-title')Commande <span
style="color:var(--ember)">#{{ str_pad($commande->id, 3, '0', STR_PAD_LEFT) }}</span>@endsection
@section('back-btn')
...
@section('topbar-actions')
  ...
  $lbls = ['en_attente' => 'En attente', 'en_preparation' => 'En préparation', 'prete' => 'Prête', 'payee' => 'Payée']; @endphp
  <span class="badge {{ $badges[$commande->statut] ?? '' }}">{{ $lbls[$commande->statut] ?? '' }}</span>
  @if($commande->statut === 'prete')
    <a href="{{ route('admin.pdf.generate', $commande->id) }}" class="btn-ember" target="_blank"><i
...
@section('content')
  ...
  <div style="display:flex;align-items:center;gap:6px;margin-bottom:20px;font-size:12.5px;color:var(--tx-tertiary);">
    ...
    <span style="color:var(--ember);">#{{ str_pad($commande->id, 3, '0', STR_PAD_LEFT) }}</span>
  </div>
    ...
    <div>
      ...
      <div class="panel glass-med" style="border-radius:var(--r-xl);margin-bottom:16px;">
        <div class="panel-body">
          <div style="font-family:'Syne',sans-serif;font-size:22px;font-weight:900;">
            Commande <span style="color:var(--ember)">#{{ str_pad($commande->id, 3, '0', STR_PAD_LEFT) }}</span>
          </div>
          <div style="font-size:12.5px;color:var(--tx-tertiary);display:flex;align-items:center;gap:5px;margin-top:3px;">
            <i class="bi bi-clock"></i>{{ $commande->created_at->format('d M Y — H:i') }}
          </div>
          @if($commande->notes)
            <div
              ...
              <i class="bi bi-chat-text" style="color:var(--ember);margin-right:6px;"></i>{{ $commande->notes }}
            </div>
          @endif
        </div>
      </div>
      ...
      <div class="panel glass" style="border-radius:var(--r-xl);margin-bottom:16px;">
        ...
        <div class="panel-body" style="padding-top:12px;">
          ...
          <div
            ...
            <span>Total</span><span style="color:var(--ember);">{{ number_format($commande->total, 0, ',', ' ') }} F</span>
          </div>
        </div>
      </div>
      ...
      @if(!in_array($commande->status, ['payee']))
        <div class="panel glass" style="border-radius:var(--r-xl);">
          ...
          <div class="panel-body">
            <form method="POST" action="{{ route('admin.orders.updateStatus', $commande->id) }}">
              @csrf @method('PATCH')
              <div class="status-select-grid">
                @php $statuts = ['en_attente' => ['icon' => 'bi-hourglass-split', 'cls' => 'sel-wait'], 'en_preparation' => ['icon' => 'bi-fire', 'cls' => 'sel-prep'], 'prete' => ['icon' => 'bi-check-circle-fill', 'cls' => 'sel-ready'], 'payee' => ['icon' => 'bi-cash-coin', 'cls' => 'sel-paid']]; @endphp
                @foreach($statuts as $val => $s)
                  @php $actif = $commande->status === $val; @endphp
    ...
    <div>
        ...
        <div class="panel-body">
          <div style="display:flex;align-items:center;gap:12px;">
            <div
              ...
              {{ mb_strtoupper(mb_substr($commande->customer_name, 0, 1)) }}
            </div>
            <div>
              <div style="font-weight:600;font-size:15px;">{{ $commande->customer_name }}</div>
              <div
                style="font-size:12.5px;color:var(--tx-tertiary);display:flex;align-items:center;gap:4px;margin-top:2px;">
                <i class="bi bi-telephone"></i>{{ $commande->customer_phone }}</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="panel-body">
              <form method="POST" action="{{ route('admin.payments.store') }}">
                ...
                <div class="field-group">
                  ...
                  <input class="field" type="number" name="montant" value="{{ $commande->total }}" required min="0">
                </div>
                <div class="field-group">
          ...
          <div class="panel-body">
            <div
              ...
              <div style="font-size:12.5px;color:var(--tx-tertiary);">
                ...
                {{ $commande->paiement->created_at->format('d/m/Y H:i') }}</div>
            </div>
          </div>
        </div>
      @endif
      ...
      @if(!in_array($commande->statut, ['payee']))
        <div class="danger-zone panel">
          ...
          <div class="panel-body">
            <form method="POST" action="{{ route('admin.orders.destroy', $commande->id) }}"
